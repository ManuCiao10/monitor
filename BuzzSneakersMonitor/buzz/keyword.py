            ...
            if response.status_code == 200:
                ...
                for product in products:
                    ...
                    if parentThread.firstRun:
                        ...
                        if url not in currentProducts and any(
                            keyword in productName.lower().split(" ")
                            ...
                            while True:
                                ...
                                print(
                                    "[KEYWORDS - {}] Getting product info.".format(pid)
                                )
                                sizes = []
                                try:
                                    response = requests.post(
                                        "https://www.buzzsneakers.gr/athlitika-papoutsia/",
                                        headers=headers,
                                        data="nbAjax=1&task=getproductdata&productId={}".format(
                                            pid
                                        ),
                                    )
                                except requests.exceptions.ConnectionError:
                                    print(
                                        "[KEYWORD - {}] Connection error.".format(pid)
                                    )
                                    continue
                                ...
                                webhook.PingServer(
                                    ...
                                    productSizes=sizes,
                                    productImage="https://www.buzzsneakers.gr/{}".format(
                                        rJSON["product"]["image"]
                                    ),
                                    sizeTitle="Sizes",
                                    productId=pid,
