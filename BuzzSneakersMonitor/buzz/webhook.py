...
def PingImageMSKU(MSKU: str, imageURL: str):
    ...
    embed = DiscordEmbed(
        title="Product has been loaded!",
        description="**{}**".format(MSKU),
        color=2524623,
    )
...
def PingServer(
    ...
    embed = DiscordEmbed(
        title=webhookTitle,
        description="**[{}]({})**".format(productName[:255], productLink),
        color=2524623,
    )
    ...
    if includePrice:
        embed.add_embed_field(
            name="Price", value="€{:.2f}".format(productPrice), inline=True
        )
    ...
    if productSizes:
        if len("\n".join([x.webhookText for x in productSizes])) > 1024:
            embed.add_embed_field(
                name=sizeTitle,
                value="\n".join(
                    [x.webhookText for x in productSizes[: len(productSizes) // 2]]
                ),
                inline=False,
            )
            embed.add_embed_field(
            ...
            embed.add_embed_field(
                name=sizeTitle,
                value="\n".join([x.webhookText for x in productSizes]),
                inline=True,
            )
